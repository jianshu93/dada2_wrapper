---
title: "DADA2 analysis of 16S rRNA gene amplicon sequencing reads--merged mode"
...
### variables inherited from main script
Fwd_pr
Rvs_pr
if(!is.null(n_threads) == "TRUE") {
  threads <- as.numeric(n_threads)
} else {
...
### set DADA2 variables
getDadaOpt()
setDadaOpt(OMEGA_A = 1e-40)
setDadaOpt(MATCH=5, MISMATCH=-4, GAP_PENALTY=-8)
...
tmp.path <- paste(input.path, "/tmp", sep="")
tmp.path
dir.create(tmp.path, recursive = TRUE)
for (p in fnFs){
    q <- gsub("_R1.fastq","_R2.fastq",p)
    s_name <- basename(p)
    s_name
    system( paste( "vsearch -fastq_mergepairs ", input.path, "/", p, " --reverse ", input.path, "/", q, " -fastqout ", tmp.path, "/", s_name, " --relabel @", sep=""))
}

fnFs_new <- sort(grep( "_R1.*\\.fastq", list.files(tmp.path), value = T ) )
fnFs_final <- file.path(tmp.path, fnFs_new)
fnFs_final
```
...
## Generate quality plots for merged reads and store in Read_QC folder.
```{r read_quality_plots}
# Create output folder
# NOTE: variable 'output.dir' containing name of output folder is 
# inherited from wrapper script dada2_wrapper.r.
cwd <- getwd()
...
for( i in 1 : length(fnFs_final)) {
  fwd.qc.plots.list[[i]] <- plotQualityProfile(fnFs_final[i])
  rm(i)
}
# Save to file
mF <- marrangeGrob( fwd.qc.plots.list, ncol=2, nrow=length(fnFs)/2, top = NULL )
ggsave(paste0(readQC.folder,"/FWD_read_plot.pdf"), mF, width = 7, height = 2.5*length(fnFs)/2, device="pdf",limitsize = FALSE)

plot_grid(fwd.qc.plots.list[[1]])
```
...
if(!is.null(Fwd_pr) & !is.null(Rvs_pr)) {
  removePrimers(fnFs_final, fnFs.noprimer, primer.fwd=Fwd_pr, primer.rev=rc(Rvs_pr), trim.fwd = TRUE,trim.rev = TRUE, orient=TRUE, verbose=TRUE)
} else {
  fnFs.noprimer <- fnFs_final
}
...
rd.counts <- as.data.frame(
  filterAndTrim(fnFs.noprimer, filtFs, truncLen=240,
              maxN=0, maxEE=1, truncQ=10, rm.phix=TRUE,
...
errF <- learnErrors(filtFs, nread=1e6, multithread=threads,randomize=TRUE)
plotErrors(errF, nominalQ=TRUE)
# Visualize the estimated error rates
# Forward
#plotErrors(errF, nominalQ=TRUE)
# Save to file 
ggsave(paste0(readQC.folder,"/Error_rates_per_sample_single.pdf"), plotErrors(errF, nominalQ=TRUE) , device = "pdf")
# Reverse
...
## print(paste("dada() starts at",Sys.time, sep = " "))
if(!pool.samples == "TRUE") {
  ...
  dadaFs <- dada(derepFs, err=errF, multithread=threads, pool=TRUE)
  mergers <- dadaFs
}
### print(paste("dada() finishes at",Sys.time, sep = " "))
rm(derepF)
```
...
colnames(track) <- c("input", "filtered", "ratio", "single", "tabled", "nonchim")
...
# Assign taxonomy:
taxa.gg13_8 <- assignTaxonomy(seqtab.nochim, "../reference_dbs_16S/gg_13_8_train_set_97.fa.gz", multithread=threads, tryRC=TRUE)
...
taxa.silva <- assignTaxonomy(seqtab.nochim, "../reference_dbs_16S/silva_nr_v128_train_set.fa.gz", multithread = threads)
...
# Assign RDP taxonomy
taxa.rdp <- assignTaxonomy(seqtab.nochim, "../reference_dbs_16S/rdp_train_set_18.fa.gz", multithread = threads)
