...
# Save to file
mF <- marrangeGrob( fwd.qc.plots.list, ncol=2, nrow=length(fnFs_final)/2+1, top = NULL )
ggsave(paste0(readQC.folder,"/FWD_read_plot.pdf"), mF, width = 7, height = 2.5*(length(fnFs_final)/2+1), device="pdf", limitsize = FALSE)
...
rd.counts <- as.data.frame(
  filterAndTrim(fnFs.noprimer, filtFs, truncLen=200,
