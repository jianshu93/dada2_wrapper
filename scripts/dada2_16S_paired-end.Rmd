...
mR <- marrangeGrob( rev.qc.plots.list, ncol=2, nrow=length(fnRs)/2, top = NULL )
ggsave(paste0(readQC.folder,"/REV_read_plot.pdf"), mR, width = 7, height = 2.5*length(fnRs)/2, device="pdf", limitsize = FALSE)
...
rd.counts <- as.data.frame(
  filterAndTrim(fnFs.noprimer, filtFs, fnRs.noprimer, filtRs, truncLen=c(225,190),
